...
# Endpoint settings
ENDPOINT_URL="https://your-ui-server/Fail2Ban-Report/endpoint/index.php"
...
for FILE in "$BLOCKLIST_DIR"/*.blocklist.json; do
  ...
  # --- POST to Endpoint ---
  log "Sending blocklist $FILE to endpoint..."
  http_status=$(curl -sS -w "%{http_code}" -o /dev/null \
  ...
  if [[ "$http_status" -ge 200 && "$http_status" -lt 300 ]]; then
    log "Blocklist $FILE successfully sent to endpoint (HTTP $http_status)."
  else
    log "ERROR: Failed to send $FILE to endpoint (HTTP $http_status)."
  fi
done
